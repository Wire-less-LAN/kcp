...
public:
	...
	virtual void send(int, const void*, int) = 0;
	virtual int recv(int, void*, int) = 0;

	int tx1, tx2;
};

// 网络延迟模拟器
class VibLatencySimulator : public LatencySimulator
{
public:

	virtual ~VibLatencySimulator() {
		clear();
	}
	...
	VibLatencySimulator(int lossmin = 10, int lossmax = 30, int rttmin = 60, int rttmax = 125, int nmax = 1000, int vib = 2): 
		r12(100), r21(100) {
		current = iclock();		
		...
		this->lostrate = (lossmax + lossmin) / 2;
		this->rtt = (rttmax + rttmin) / 2;
		this->nmax = nmax;
		this->vib = vib;
		tx1 = tx2 = 0;
	}
...
protected:
	...
	void viberate() {
		...
		rtt += vib * (rand() % 2 ? 1 : -1) * (rttmax - rttmin) * 100;
		rtt = std::max(rtt, rttmin);
		rtt = std::min(rtt, rttmax);
	}
};

class OldLatencySimulator : public LatencySimulator
{
public:

	virtual ~OldLatencySimulator() {
		clear();
	}
	...
	// rttmin：rtt最小值，默认 60
	// rttmax：rtt最大值，默认 125
	OldLatencySimulator(int lostrate = 10, int rttmin = 60, int rttmax = 125, int nmax = 1000): 
		r12(100), r21(100) {
		current = iclock();		
